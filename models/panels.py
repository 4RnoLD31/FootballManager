...
from models.statistics import ShowBalance, Statistics
from models.debug import AllObjects, ChangeBalance
...
class ChangeAvatar:
    def __init__(self, player):
        ...
        for element in range(len(listdir(path=f"{working_directory}\\assets\\avatars\\"))):
            self.path = f"{working_directory}\\assets\\avatars\\{element}.png"
            ...
            self.paths.append(f"{working_directory}\\assets\\avatars\\{element}.png")
...
class ChangeName:
    def __init__(self, player):
        ...
        self.l_name = Label(self.window, text=f"Старое имя: {self.player.name}", font="MiSans 25")
    ...
    def __set_name__(self, name):
        ...
        if self.player == utils.constants.PL1:
            utils.constants.PL1.name = self.name
        else:
            utils.constants.PL2.name = self.name
        panels_initialize()
...
def panels_initialize():
    ...
    property = Menu(utils.constants.main_window, tearoff=0)
    objects_menu = Menu(utils.constants.main_window, tearoff=0)
    sell = Menu(utils.constants.main_window, tearoff=0)
    menu.add_cascade(label="Файл", menu=file_menu)
    menu.add_cascade(label="Изменить", menu=change_menu)
    menu.add_cascade(label="Имущество", menu=property)
    ...
    property.add_cascade(label="Продажа", menu=sell)
    ...
    if utils.constants.PL1 is not None:
        debug_menu.add_command(label="Изменить баланс", command=ChangeBalance)
        statistic_menu.add_command(label="Баланс", command=ShowBalance)
        statistic_menu.add_command(label="Основная статистика", command=Statistics)
        sell.add_command(label=f"Игрок 1 - {utils.constants.PL1.name}", command=lambda: Sell(utils.constants.PL1, None, None, None, None))
        sell.add_command(label=f"Игрок 2 - {utils.constants.PL2.name}", command=lambda: Sell(utils.constants.PL2, None, None, None, None))
