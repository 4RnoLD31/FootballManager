...
class Player:
    ...
    def check_withdrawal(self, summary, economist=True):
        ...
        if self.economist_level() != 0:
            self.summary = round(self.summary - self.summary * economist_minus_level[self.economist_level()]) // 10000 * 10000
    ...
    def summary_check(self, summary_old, type="Minus", economist=True):
        ...
        # print(c_info(f"Check for {self.name}: before {self.summary_old} after {self.summary}"))
        return self.summary
    ...
    def withdrawal(self, summary, type="Balance", economist=True):
        ...
        if self.type == "Balance":
            if self.summary > self.balance:
                print(c_failed(f"{self.name} was unable to withdraw {self.summary} from {self.type}"))
                return False
            self.balance -= self.summary
        elif self.type == "Income":
            self.income -= self.summary
        # print(c_successful(f"{self.name} withdrew {self.summary} from {self.type}"))
    ...
    def deposit(self, summary, type="Balance", economist=True):
        ...
        if self.type == "Balance":
            ...
            self.income += self.summary
        # print(c_info(f"{self.name}'s balance has been deposited by {self.summary} to {self.type}"))
    ...
    def full_leagues(self):
        ...
        for element in clubs.keys():
            if clubs[element].owner == self:
                if clubs[element].league == "EPL":
                    self.apl += 1
                elif clubs[element].league == "La Liga":
                    self.la_liga += 1
                elif clubs[element].league == "Seria A":
                    self.seria_a += 1
                elif clubs[element].league == "Bundesliga":
                    self.bundesleague += 1
                elif clubs[element].league == "RPL":
                    self.rpl += 1
        if self.apl == 3:
    ...
    def need_money(self, need):
        ...
        self.l_property = Label(utils.constants.main_window, text=f"Имущество игрока {self.name}", font="MiSans 50")
    ...
    def where_can_i_have_a_footballer(self, without=True):
        ...
        for element in clubs.keys():
            if clubs[element].owner == self:
                self.clubs.append(clubs[element])
        if without:
    ...
    def where_can_i_have_a_coach(self, without=True):
        ...
        for element in clubs.keys():
            if clubs[element].owner == self and clubs[element].footballer is not None and clubs[element].league in self.l_full_leagues:
                self.clubs.append(clubs[element])
        if len(self.clubs) < 3:
            return None
        while len(self.clubs) % 3 != 0 or len(self.l_full_leagues) * 3 > len(self.clubs):
            ...
            self.to_delete = []
            for element in clubs.keys():
                if clubs[element].owner == self and clubs[element].footballer is None and clubs[element].league in self.l_full_leagues:
                    self.impostor_league = clubs[element].league
                    break
            for element in self.clubs:
    ...
    def where_can_i_have_a_manager(self, without=True):
        ...
        self.l_full_leagues = self.full_leagues()
        for element in clubs.keys():
            if clubs[element].owner == self and clubs[element].footballer is not None and clubs[element].coach is not None and clubs[element].league in self.l_full_leagues:
                self.clubs.append(clubs[element])
        if len(self.clubs) < 3:
            return None
        while len(self.clubs) % 3 != 0 or len(self.l_full_leagues) * 3 > len(self.clubs):
            ...
            self.to_delete = []
            for element in clubs.keys():
                if clubs[element].owner == self and clubs[element].coach is None and clubs[element].league in self.l_full_leagues:
                    self.impostor_league = clubs[element].league
                    break
            for element in self.clubs:
                if element.league == self.impostor_league:
