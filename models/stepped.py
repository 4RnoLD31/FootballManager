...
def stepped_on_club(player, club):
    if club.owner == player:
        ...
        text_on_center(player.name + " наступил на свободный клуб " + club.name + ". Хочешь купить его?", "MiSans 30")
        b_info = Button(utils.constants.main_window, text="Информация о клубе", font="MiSans 30", command=club.info)
        b_info.place(x=50, y=700)
        b_buy = Button(utils.constants.main_window, text="Купить", font="MiSans 30", command=lambda: print())
...
def stepped_on_tv_company(player, tv_company):
    if tv_company.owner == player:
        text_on_center(player.name + " наступил на свою телекомпанию " + tv_company.name, "MiSans 35")
        ...
        text_on_center(player.name + " наступил на свободную телекомпанию " + tv_company.name + ". Хочешь купить ее?", "MiSans 30")
        b_info = Button(utils.constants.main_window, text="Информация о телекомпании", font="MiSans 30", command=nothing)
        b_info.place(x=50, y=700)
        ...
        text_on_center(player.name + " наступил на телекомпанию " + tv_company.name + ". Им владеет " + tv_company.owner.name, "MiSans 40")