...
class OtherBuy:
    ...
    def __buy__(self):
        ...
        self.b_confirm = const.Button(const.main_window, text="Подтвердить", font="MiSans 40", command=self.__confirm__)
        self.b_confirm.custom_place(x=200, y=600, width=360, height=100)
        self.b_cancel = const.Button(const.main_window, text="Отменить", font="MiSans 40", command=self.back)
        self.b_cancel.custom_place(x=1040, y=600, width=360, height=100)
...
class LEnough:
    def __init__(self, player, need_money):
        ...
        self.window.custom_geometry("360x60+780+940")
        self.window.title("НЕДОСТАТОЧНО")
        self.window.resizable(width=False, height=False)
        self.l_enough = const.Label(self.window, font="MiSans 40")
    ...
    def switch_on(self):
        self.window.custom_geometry("300x60+810+940")
...
class Transfer:
    def __init__(self, player, back):
        ...
        self.player = player
        const.clear()
        self.l_logo = const.Label(const.main_window, text=f"Трансфер для игрока {self.player.name}", font="MiSans 40")
        self.l_logo.custom_place(x=800 - self.l_logo.winfo_reqwidth() / 2, y=0)
        self.b_manager = const.Button(const.main_window, text="Менеджеры", font="MiSans 35", command=self.__manager__)
        self.b_coach = const.Button(const.main_window, text="Тренеры", font="MiSans 35", command=self.__coach__)
        self.b_footballer = const.Button(const.main_window, text="Футболисты", font="MiSans 35", command=self.__footballer__)
        self.b_manager.custom_place(x=100, y=170, width=452, height=100)
        self.b_coach.custom_place(x=572, y=170, width=452, height=100)
        self.b_footballer.custom_place(x=1044, y=170, width=452, height=100)
        self.b_back = const.Button(const.main_window, text="Назад", font="MiSans 20", command=self.__back__)
        self.b_back.custom_place(x=20, y=730, height=50)
...
class TransferItem:
    ...
    def __menu__(self):
        ...
        if self.type == "Manager":
            self.window.custom_geometry("500x800+160+115")
            self.found = self.player.search_bought_managers()
            self.l_logo = const.Label(self.window, text="Выберите менеджера для трансфера", font="MiSans 18")
            ...
            self.window.custom_geometry("500x800+710+115")
            self.found = self.player.search_bought_coaches()
            self.l_logo = const.Label(self.window, text="Выберите тренера для трансфера", font="MiSans 18")
            for element in self.found:
                if element.dead is True or element.flu is not None or element.strike is not None:
                    self.name_found.append(element)
        elif self.type == "Footballer":
            self.window.custom_geometry("500x800+1260+115")
            self.found = self.player.search_bought_footballers()
            self.l_logo = const.Label(self.window, text="Выберите футболиста для трансфера", font="MiSans 18")
            for element in self.found:
                if element.dead is True or element.flu is not None:
                    self.name_found.append(element)
        ...
        if not self.found:
            self.l_nothing = const.Label(self.window, text="Ничего не найдено", font="MiSans 40")
            self.l_nothing.place(x=self.window.winfo_reqwidth() / 2 - self.l_nothing.winfo_reqwidth() / 2, y=self.window.winfo_reqheight() / 2 - self.l_nothing.winfo_reqheight() // 2)
            self.window.after(3000, self.window.destroy)
            return
        self.l_logo.custom_place(x=250 - self.l_logo.winfo_reqwidth() / 2, y=0)
        self.cb_found = const.Combobox(self.window, values=self.name_found, font="MiSans 20", state="readonly")
        self.cb_found.bind("<<ComboboxSelected>>", lambda event, source="found": self.__show__(source))
        self.cb_found.custom_place(x=50, y=640, height=50, width=400)
        self.b_confirm = const.Button(self.window, text="Подтвердить", font="MiSans 30", command=self.__to__)
        self.b_confirm.custom_place(x=250 - (self.b_confirm.winfo_reqwidth() / 2), y=700)
    ...
    def __cb_found__(self):
        ...
        if self.type == "Manager":
            ...
            try:
                ...
                self.text = "Клуб: Отсутствует"
        self.l_club = const.Label(self.window, text=self.text, font="MiSans 20")
        self.l_club.custom_place(x=250 - self.l_club.winfo_reqwidth() / 2, y=590)
    ...
    def __to__(self):
        ...
        if not self.clubs:
            self.l_nothing = const.Label(self.window, text="Нет доступных клубов", font="MiSans 30")
            self.l_nothing.custom_place(x=250 - self.l_nothing.winfo_reqwidth() / 2, h=400 - self.l_nothing.winfo_reqheight() // 2)
            self.window.after(2000, self.window.destroy)
            return
        self.l_logo.configure(text="Выберите клуб", font="MiSans 30")
        self.l_logo.custom_place(x=250 - self.l_logo.winfo_reqwidth() / 2, y=0)
        self.cb_clubs = const.Combobox(self.window, values=self.name_clubs, font="MiSans 20", state="readonly")
        self.cb_clubs.bind("<<ComboboxSelected>>", lambda event, type="clubs": self.__show__(type))
        self.cb_clubs.custom_place(x=50, y=640, height=50, width=400)
        self.b_confirm = const.Button(self.window, text="Подтвердить", font="MiSans 30", command=self.__are_you_sure__)
        self.b_confirm.custom_place(x=250 - (self.b_confirm.winfo_reqwidth() / 2), y=700)
        self.b_back = const.Button(self.window, text="Назад", font="MiSans 18", command=self.__menu__)
        self.b_back.custom_place(x=7, y=730, height=50)
    ...
    def __are_you_sure__(self):
        ...
        self.l_text = const.Label(self.window, text=self.text, font="MiSans 20", wraplength=500)
        self.l_text.custom_place(x=250 - self.l_text.winfo_reqwidth() / 2, y=(400 - self.l_text.winfo_reqheight() // 2) - 50)
        self.b_back = const.Button(self.window, text="Назад", font="MiSans 20", command=self.__to__)
        self.b_back.custom_place(x=50, y=680, width=190, height=100)
        self.b_confirm = const.Button(self.window, text="Подтвердить", font="MiSans 20", command=self.__confirm__)
        self.b_confirm.custom_place(x=260, y=680, width=190, height=100)
    ...
    def __confirm__(self):
        ...
        for element in self.window.winfo_children():
            element.destroy()
        self.l_logo = const.Label(self.window, text="Успешно", font="MiSans 40")
        self.l_logo.custom_place(x=250 - self.l_logo.winfo_reqwidth() / 2, y=400 - self.l_logo.winfo_reqheight() // 2)
        self.window.after(2000, self.window.destroy)
...
class BuyItems:
    ...
    def __next__(self):
        ...
        if self.type == "Footballer":
            ...
            self.l_logo = const.Label(self.window, text="Выберите уровень футболиста", font="MiSans 25")
            self.l_logo.custom_place(x=250 - (self.l_logo.winfo_reqwidth() / 2), y=0)
            ...
            if not self.available_clubs:
                ...
                self.window.destroy()
                return
            self.b_10_level = const.Button(self.window, text="10", font="MiSans 40", command=lambda: self.menu("Footballer", 10))
            self.b_10_level.custom_place(x=20, y=150, width=460, height=80)
            self.b_8_level = const.Button(self.window, text="8", font="MiSans 40", command=lambda: self.menu("Footballer", 8))
            self.b_8_level.custom_place(x=20, y=250, width=460, height=80)
            self.b_6_level = const.Button(self.window, text="6", font="MiSans 40", command=lambda: self.menu("Footballer", 6))
            self.b_6_level.custom_place(x=20, y=350, width=460, height=80)
            self.b_4_level = const.Button(self.window, text="4", font="MiSans 40", command=lambda: self.menu("Footballer", 4))
            self.b_4_level.custom_place(x=20, y=450, width=460, height=80)
            self.b_2_level = const.Button(self.window, text="2", font="MiSans 40", command=lambda: self.menu("Footballer", 2))
            self.b_2_level.custom_place(x=20, y=550, width=460, height=80)
        elif self.type == "Coach":
            self.window.custom_geometry("500x800+710+115")
            self.window.title("Покупка тренеров")
            self.l_logo = const.Label(self.window, text="Выберите уровень тренера", font="MiSans 25")
            self.l_logo.custom_place(x=250 - (self.l_logo.winfo_reqwidth() / 2), y=0)
            ...
            if not self.available_clubs:
                ...
                self.window.destroy()
                return
            self.b_3_level = const.Button(self.window, text="3", font="MiSans 40", command=lambda: self.menu("Coach", 3))
            self.b_3_level.custom_place(x=20, y=150, width=460, height=80)
            self.b_2_level = const.Button(self.window, text="2", font="MiSans 40", command=lambda: self.menu("Coach", 2))
            self.b_2_level.custom_place(x=20, y=250, width=460, height=80)
            self.b_1_level = const.Button(self.window, text="1", font="MiSans 40", command=lambda: self.menu("Coach", 1))
            self.b_1_level.custom_place(x=20, y=350, width=460, height=80)
        elif self.type == "Manager":
            self.window.custom_geometry("500x800+160+115")
            self.window.title("Покупка менеджеров")
            self.l_logo = const.Label(self.window, text="Выберите тип менеджера", font="MiSans 25")
            self.l_logo.custom_place(x=250 - (self.l_logo.winfo_reqwidth() / 2), y=0)
            ...
            if not self.available_clubs:
                ...
                self.window.destroy()
                return
            self.b_sheikh = const.Button(self.window, text="Шейхи", font="MiSans 40", command=lambda: self.menu("Sheikh"))
            self.b_sheikh.custom_place(x=20, y=150, width=460, height=80)
            self.b_former_footballer = const.Button(self.window, text="Бывшие футболисты", font="MiSans 30", command=lambda: self.menu("Former Footballer"))
            self.b_former_footballer.custom_place(x=20, y=250, width=460, height=80)
            self.b_economist = const.Button(self.window, text="Экономисты", font="MiSans 40", command=lambda: self.menu("Economist"))
            self.b_economist.custom_place(x=20, y=350, width=460, height=80)

    def menu(self, type, power=None):
        if type == "Footballer":
            self.found = search_available_footballers(power)
            self.l_logo = const.Label(self.window, text="Футболисты " + str(power) + " уровня", font="MiSans 25")
            self.l_logo.custom_place(x=250 - (self.l_logo.winfo_reqwidth() / 2), y=0)
        elif type == "Coach":
            self.found = search_available_coaches(power)
            self.l_logo = const.Label(self.window, text="Тренеры " + str(power) + " уровня", font="MiSans 25")
            self.l_logo.custom_place(x=250 - (self.l_logo.winfo_reqwidth() / 2), y=0)
        elif type == "Former Footballer":
            self.found = search_available_managers(type)
            self.l_logo = const.Label(self.window, text="Бывшие футболисты", font="MiSans 30")
            self.l_logo.custom_place(x=250 - (self.l_logo.winfo_reqwidth() / 2), y=0)
        elif type == "Economist":
            self.found = search_available_managers(type)
            self.l_logo = const.Label(self.window, text="Экономисты", font="MiSans 30")
            self.l_logo.custom_place(x=250 - (self.l_logo.winfo_reqwidth() / 2), y=0)
        elif type == "Sheikh":
            self.found = search_available_managers(type)
            self.l_logo = const.Label(self.window, text="Шейхи", font="MiSans 30")
            self.l_logo.custom_place(x=250 - (self.l_logo.winfo_reqwidth() / 2), y=0)
        self.y = 55
        ...
        for element in self.window.winfo_children():
            if element != self.l_logo:
                element.destroy()
        self.b_back = const.Button(self.window, text="Назад", font="MiSans 20", command=self.__next__)
        self.b_back.custom_place(x=20, y=730, height=50)
        for element in range(0, len(self.found)):
            self.picked_bool.append(tk.BooleanVar())
        for element in range(0, len(self.found)):
            self.check_buttons.append(
                const.Checkbutton(self.window, variable=self.picked_bool[element], text=self.found[element].name,
                               font="MiSans 25"))
            self.check_buttons[element].config(command=self.clicked)
            self.check_buttons[element].custom_place(x=10, y=self.y, height=50)
            self.y += 50
        self.b_buy = const.Button(self.window, text="Купить", font="MiSans 40", command=self.buy)
        self.b_buy.custom_place(x=250 - (self.b_buy.winfo_reqwidth() / 2), y=710, height=80)
    ...
    def buy(self):
        ...
        self.l_bought_first = const.Label(self.window, text=self.picked.name, font="MiSans 25")
        self.l_bought_first.custom_place(x=250 - (self.l_bought_first.winfo_reqwidth() / 2), y=60)
        self.l_bought = const.Label(self.window, text="Куплен за " + str(self.price_bought), font="MiSans 30")
        self.l_bought.custom_place(x=250 - (self.l_bought.winfo_reqwidth() / 2), y=110)
        self.window.after(3000, self.choice_club)

    def choice_club(self):
        for widget in self.window.winfo_children():
            widget.destroy()
        self.l_logo = const.Label(self.window, text="В какой клуб?", font="MiSans 40")
        self.l_logo.custom_place(x=250 - (self.l_logo.winfo_reqwidth() / 2), y=0)
        self.text = ""
        self.club_names = []
        for element in self.available_clubs:
            self.club_names.append(element.name)
        self.list = const.Combobox(self.window, values=self.club_names, font="MiSans 20", state="readonly")
        self.list.custom_place(x=250 - (self.list.winfo_reqwidth() / 2), y=640)
        self.b_confirm = const.Button(self.window, text="Подтвердить", font="MiSans 30", command=self.transfer)
        self.b_confirm.custom_place(x=250 - (self.b_confirm.winfo_reqwidth() / 2), y=700)
    ...
    def transfer(self):
        ...
        if isinstance(self.picked, footballer.Footballer):
            self.l_successfully = const.Label(self.window, text="Футболист " + self.picked.name + " успешно перешел в клуб " + self.picked.club.name, font="MiSans 20", wraplength=500)
            self.available_clubs = self.player.where_can_i_have_a_footballer()
        elif isinstance(self.picked, coach.Coach):
            self.l_successfully = const.Label(self.window, text="Тренер " + self.picked.name + " успешно перешел в клуб " + self.picked.club.name, font="MiSans 20", wraplength=500)
            self.available_clubs = self.player.where_can_i_have_a_coach()
        elif isinstance(self.picked, manager.Manager):
            self.l_successfully = const.Label(self.window, text="Менеджер " + self.picked.name + " успешно перешел в клуб " + self.picked.club.name, font="MiSans 20", wraplength=500)
            self.available_clubs = self.player.where_can_i_have_a_manager()
        self.l_successfully.custom_place(x=250 - self.l_successfully.winfo_reqwidth() / 2, y=80)
...
class Buy:
    def __init__(self, player, next_step):
        ...
        self.l_logo = const.Label(self.window, text="Трансферное окно игрока " + self.player.name, font="MiSans 35")
        self.b_back = const.Button(self.window, text="Назад", font="MiSans 20", command=self.__back__)
        self.b_back.custom_place(x=20, y=730, height=50)
        ...
        for element in self.names.keys():
            self.buttons.append(const.Button(self.window, text=element, font="MiSans 35"))
            self.buttons[self.index].custom_place(x=self.names[element], y=self.y, width=452, height=100)
            self.index += 1
...
class SellItems:
    def __init__(self, type, player, transfer_market, need_money, l_enough):
        ...
        if self.type == "Footballer":
            self.window.custom_geometry("300x900+1470+30")
            self.window.title("Футболисты игрока " + self.player.name)
            self.l_logo = const.Label(self.window, text="Футболисты", font="MiSans 30")
            self.found = self.player.search_bought_footballers()
        elif self.type == "Coach":
            self.window.custom_geometry("300x900+1140+30")
            self.window.title("Тренеры игрока " + self.player.name)
            self.l_logo = const.Label(self.window, text="Тренеры", font="MiSans 30")
            self.found = self.player.search_bought_coaches()
        elif self.type == "Manager":
            self.window.custom_geometry("300x900+810+30")
            self.window.title("Менеджеры игрока " + self.player.name)
            self.l_logo = const.Label(self.window, text="Менеджеры", font="MiSans 30")
            self.found = self.player.search_bought_managers()
        elif self.type == "TV":
            self.window.custom_geometry("300x900+480+30")
            self.window.title("Телекомпании игрока " + self.player.name)
            self.l_logo = const.Label(self.window, text="Телекомпании", font="MiSans 30")
            self.found = self.player.search_owned_TVs()
        elif self.type == "Club":
            self.window.custom_geometry("300x900+150+30")
            self.window.title("Клубы игрока " + self.player.name)
            self.l_logo = const.Label(self.window, text="Клубы", font="MiSans 30")
        ...
        self.l_logo.pack(side="top")
        if not self.found:
            self.l_nothing = const.Label(self.window, text="Пусто", font="MiSans 50")
            self.l_nothing.custom_place(x=self.window.winfo_reqwidth() / 2 - self.l_nothing.winfo_reqwidth() / 2, y=self.window.winfo_reqheight() / 2 - (self.l_nothing.winfo_reqheight() / 2))
            self.window.after(4000, self.window.destroy)
            return
        self.b_sell = const.Button(self.window, text="Продать", font="MiSans 40", command=self.sell)
        self.b_sell.custom_place(x=150 - (self.b_sell.winfo_reqwidth() / 2), y=800, height=80)
        self.x = 10
        self.y = 70
        self.will_sell = []
        self.picked = []
        ...
        for element in range(0, len(self.found)):
            self.check_buttons.append(
                const.Checkbutton(self.window, variable=self.picked[element], text=self.found[element].name,
                               font="MiSans 20"))
            self.check_buttons[element].config(command=self.clicked)
            self.check_buttons[element].custom_place(x=10, y=self.y, height=50)
            self.y += 35
    ...
    def sell(self):
        ...
        for element in self.will_sell:
            ...
            self.l_sold_first = const.Label(self.window, text=element.name, font="MiSans 15")
            self.l_sold_first.custom_place(x=150 - (self.l_sold_first.winfo_reqwidth() / 2), y=self.y)
            self.y += 40
            self.l_sold = const.Label(self.window, text="Продан за " + str(self.price_sold), font="MiSans 20")
            const.sum -= self.price_sold
            self.l_sold.custom_place(x=150 - (self.l_sold.winfo_reqwidth() / 2), y=self.y)
            self.y += 50
        self.window.after(3000, self.window.destroy)
...
class Sell:
    def __init__(self, player, next_step, full_screen=True, transfer_market=None, need_money=None):
        ...
        for element in self.names.keys():
            self.buttons.append(const.Button(self.window, text=element, font="MiSans 35"))
            self.buttons[self.index].custom_place(x=self.names[element], y=self.y)
            self.index += 1
