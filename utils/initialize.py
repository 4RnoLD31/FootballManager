...
if not os.path.exists(f"{working_directory}\\settings.ini"):
    settings_file = ConfigParser()
    with open(path_to_settings, "w") as settings:
        settings_file.write(settings)

if not os.path.exists(f"{working_directory}\\\\tmp"):
    os.mkdir(f"{working_directory}\\tmp")


def initialize():
    database_file = ConfigParser()
    database_file.read(f"{working_directory}//configs//database.cfg")
    ...
    for index in range(len(tv_name)):
        utils.constants.TVs[tv_name[index]] = TVCompany(tv_name[index])
    # Создайте словарь, разбивая каждую пару на ключ и значение
    for index in range(len(club_name)):
        utils.constants.clubs[club_name[index]] = Club(club_name[index], int(club_price[index]), club_league[index], club_color[index], club_codename[index])
